...
def countdown(from_value: int, tts=None):
...
def count_reps(pose_estimator: PoseEstimator, tts, exercises: dict, exercise: str, count_to: int):
  ...
  if time_based:
    ...
    while counter < count_to:
      if pose_estimator.keypoints_available:
        ...
        _angles.append(joint_angle_estimator.estimate(pose_estimator.get_keypoints()))
        _mse.append(curr_mse)
        _rule.append(thresholded)
...
def main():
  # This fixes a bug with QT and OpenCV.
  ...
  count_reps(pose_estimator, None, exercises, 'squats', 10)
  pose_estimator.stop()
